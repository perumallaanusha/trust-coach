...
function Button({ className, variant = 'default', size = 'default', asChild = false, children, ...props }: ButtonPrimitive.Props & VariantProps<typeof buttonVariants> & { asChild?: boolean }) {
  ...
  if (asChild) {
    if (!isValidElement(children)) throw new Error('Button with asChild must have a single element child.')
    const child = children as ReactElement<{ className?: string; ['data-slot']?: string }>
    return cloneElement(child, { ...props, 'data-slot': 'button', className: cn(classes, child.props.className) })
  }
